...
import {
  ...
  companyPolicies, policyAcknowledgments, overtimeRequests, compOffRequests, onDutyRequests, journalEntries,
...
export class DatabaseStorage implements IStorage {
  ...
  async deletePayrollByMonthForEmployees(month: string, year: number, employeeIds: number[]): Promise<void> {
    ...
    const existingRecords = await db.select({ id: payroll.id }).from(payroll).where(and(eq(payroll.month, month), eq(payroll.year, year), inArray(payroll.employeeId, employeeIds)));
    if (existingRecords.length > 0) {
      const payrollIds = existingRecords.map(r => r.id);
      await db.delete(journalEntries).where(inArray(journalEntries.payrollId, payrollIds));
    }
