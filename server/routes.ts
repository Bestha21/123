  ...
  app.post("/api/employees/bulk-update", async (req, res) => {
    try {
      ...
      const ALLOWED_TEXT_FIELDS = new Set([
        ...
        "qualificationScore", "secondQualificationScore", "noticeBuyoutDuration", "accessRole","email","onboardingStatus"
      ]);
      ...
      const BLOCKED_FIELDS = new Set(["id", "employeeCode", "password", "profileImageUrl", "createdAt", "updatedAt", ]);
...
app.delete("/api/departments/:id", async (req, res) => {
  ...
  for (const admin of hrAdmins) {
    sendNotificationEmail(
      ...
      `<p>${empName} (${employee.employeeCode || ''}) has requested the following profile changes:</p>
       <ul>${fieldList}</ul>
    ...
    ).catch((e) => console.error(`[Email Error] Admin notification failed for ${admin.email}:`, e.message));
  }
} catch (e: any) { 
  console.error("Profile change notification error:", e.message); 
}
  ...
  app.get("/api/journal-entries", async (req, res) => {
    try {
      ...
      if (month) {
        ...
        const monthIdx = parseInt(String(month), 10) - 1;
        const monthName = monthNames[monthIdx] || month;
        conditions.push(`(month=$${params.length + 1} OR month=$${params.length + 2} OR month=$${params.length + 3})`);
        params.push(String(month), monthNum, monthName);
      }
