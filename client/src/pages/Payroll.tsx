...
function PayslipView({ record, employee, onClose, entityName, entityAddress, entityDetails }: { record: Payroll; employee: Employee | undefined; onClose: () => void; entityName?: string; entityAddress?: string; entityDetails?: string }) {
  ...
  const handlePrint = () => {
    ...
    const earnRowsHtml = earningsRows.map(row =>
      `<div style="display:grid;grid-template-columns:1fr 90px 90px;gap:8px;padding:3px 0;font-size:12px;">
        <span>${row.label}</span>
          ...
          <div style="max-width:800px;margin:0 auto;border:2px solid #333;overflow:hidden;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;">
              <div style="padding:10px 12px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 90px 90px;gap:8px;font-size:10px;font-weight:bold;color:#666;padding-bottom:4px;border-bottom:1px solid #eee;margin-bottom:4px;">
                  <span>Component</span>
                  <span style="text-align:right;">Gross/Mo</span>
                  <span style="text-align:right;">Amount</span>
                </div>
                ${earnRowsHtml}
                <div style="display:grid;grid-template-columns:1fr 90px 90px;gap:8px;padding:6px 0;font-size:13px;font-weight:bold;border-top:1px solid #333;margin-top:8px;">
              ...
              <div className="payslip border-2 border-foreground/30 rounded-md overflow-hidden">
                ...
                <div className="grid grid-cols-2">
                  <div className="p-3">
                    ...
                    <div className="grid grid-cols-[1fr_90px_90px] gap-2 text-[10px] font-semibold text-muted-foreground pb-1 border-b border-dashed mb-1">
                      <span>Component</span>
                    ...
                    <div className="space-y-1">
                      {earningsRows.map(row => (
                        <div key={row.label} className="grid grid-cols-[1fr_90px_90px] gap-2 text-xs">
                          <span>{row.label}</span>
                    ...
                    <div className="grid grid-cols-[1fr_90px_90px] gap-2 text-sm font-bold mt-3 pt-2 border-t">
                      <span>Total Earnings</span>
...
function RunPayrollDialog({ open, onClose, employees }: { open: boolean; onClose: () => void; employees: Employee[] }) {
  ...
  const { data: salaryStructures } = useQuery<SalaryStructure[]>({
    queryKey: ["/api/salary-structures", selectedEntityIds],
    queryFn: async () => {
      const res = await fetch(`/api/salary-structures${entityFilterParam ? `?${entityFilterParam}` : ''}`, { credentials: "include" });
...
export default function PayrollPage() {
  ...
  const handleExportBankTemplate = () => {
    ...
    const rows = processedRecords.map((record: any) => {
      ...
      if (isHDFC) {
        txnType = 'I';
      } else if (netPay > 200000 || isConsultant) {
        txnType = 'R';
      }
