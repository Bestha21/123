...
function OrgNodeCard({ node, departments, level, expandedNodes, toggleNode, searchQuery }: {
  ...
  const levelColors = [
    { bg: "bg-primary", ring: "ring-primary/20", badge: "bg-primary/10 text-primary", line: "bg-primary" },
    { bg: "bg-blue-600", ring: "ring-blue-200", badge: "bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300", line: "bg-blue-500" },
    { bg: "bg-emerald-600", ring: "ring-emerald-200", badge: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300", line: "bg-emerald-500" },
    { bg: "bg-purple-600", ring: "ring-purple-200", badge: "bg-purple-50 text-purple-700 dark:bg-purple-950/30 dark:text-purple-300", line: "bg-purple-500" },
    { bg: "bg-orange-600", ring: "ring-orange-200", badge: "bg-orange-50 text-orange-700 dark:bg-orange-950/30 dark:text-orange-300", line: "bg-orange-500" },
    { bg: "bg-pink-600", ring: "ring-pink-200", badge: "bg-pink-50 text-pink-700 dark:bg-pink-950/30 dark:text-pink-300", line: "bg-pink-500" },
  ];
  const color = levelColors[Math.min(level, levelColors.length - 1)];
  const childColor = levelColors[Math.min(level + 1, levelColors.length - 1)];
  ...
  return (
    <div className="flex flex-col items-center" data-testid={`org-node-${emp.id}`}>
      ...
      {hasChildren && isExpanded && filteredChildren.length > 0 && (
        <>
          <div className={`w-0.5 h-6 ${color.line} rounded-full`} />
          <div className="relative">
            {filteredChildren.length > 1 && (
              <div className={`absolute top-0 left-1/2 -translate-x-1/2 h-0.5 ${childColor.line} rounded-full`} style={{
            ...
            <div className="flex gap-4 flex-wrap justify-center">
              {filteredChildren.map(child => (
                <div key={child.employee.id} className="flex flex-col items-center">
                  <div className={`w-0.5 h-6 ${childColor.line} rounded-full`} />
                  <OrgNodeCard
                    node={child}
...
function DeptNodeCard({ node, level, expandedDepts, toggleDept, searchQuery, allEmployees }: {
  ...
  const levelColors = [
    { bg: "bg-primary", line: "bg-primary" },
    { bg: "bg-blue-600", line: "bg-blue-500" },
    { bg: "bg-emerald-600", line: "bg-emerald-500" },
    { bg: "bg-purple-600", line: "bg-purple-500" },
    { bg: "bg-orange-600", line: "bg-orange-500" },
    { bg: "bg-pink-600", line: "bg-pink-500" },
  ];
  const colorObj = levelColors[Math.min(level, levelColors.length - 1)];
  const bgColor = colorObj.bg;
  const childColorObj = levelColors[Math.min(level + 1, levelColors.length - 1)];
  ...
  return (
    <div className="flex flex-col items-center" data-testid={`dept-node-${dept.id}`}>
      ...
      {hasContent && isExpanded && (
        <>
          <div className={`w-0.5 h-6 ${colorObj.line} rounded-full`} />
          <div className="relative">
            <div className="flex gap-4 flex-wrap justify-center">
              {filteredChildren.map(child => (
                <div key={child.department.id} className="flex flex-col items-center">
                  <div className={`w-0.5 h-6 ${childColorObj.line} rounded-full`} />
                  <DeptNodeCard
                    node={child}
                    level={level + 1}
